...
class Preprocessor:
    def __init__(self,
        ...
        self.stock_data_path: Path = config.data_path / self.config.save_path / "daily_stock_data"
        self.image_file_path: Path = config.data_path / self.config.save_path / "stock_chart_image"
    ...
    def _generate_chart_image(self,
        ...
        for i in tqdm(range(len(df) - self.config.window - self.config.prediction_day), desc=f"Data {file_prefix}"):
            ...
            save_path = self.image_file_path / f"{file_prefix}_{i}_{label}.jpg"
